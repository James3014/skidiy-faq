...
4. **Analytics 分析儀表板**
   ...
   - 每日請求趨勢圖表
   - 成本預警與監控
   - FAQ 點擊分析（語系、時間範圍、本地記錄）
   - 雪場瀏覽分析（地區、互動類型、趨勢）
   - 反饋分析（滿意度、原因分析、Top/Bottom 內容）
   - 多層降級策略（API → 計算 → localStorage → 空狀態）
...
**負面反饋原因**:
- 📉 原因分佈（找錯了問題/內容不完整/需要更多細節）
- 📊 百分比統計
- 從 recent API 計算降級

**滿意度分析**:
- ⭐ Top 10 滿意度最高內容（FAQ/雪場，顯示中文標題）
- ⚠️ Bottom 10 滿意度最低內容（含警告提示）
- 🔔 滿意度 <50% 內容警告
- 📊 滿意率顏色編碼（綠/黃/紅）
...
**多層降級策略**:
1. Stats API → 計算自 Recent API → localStorage
2. Top/Bottom Items API → 計算自 Recent API → 空狀態
3. Reasons API → 計算自 Recent API → 空狀態
4. 所有反饋數據顯示 FAQ 中文標題
...
Analytics 系統採用多層降級策略，即使後端 API 不可用，仍可顯示分析數據：

1. **優先級 1**: 從後端 API 獲取聚合統計
2. **優先級 2**: 從 Recent API 計算統計數據（適用於反饋分析）
3. **優先級 3**: 從 localStorage 讀取本地追蹤數據
4. **優先級 4**: 顯示友好的空狀態提示

所有用戶互動（點擊、瀏覽、反饋）都會自動保存到 localStorage，確保數據不丟失。
...
**新功能**:
- ✅ **FAQ 點擊分析** - 完整的 FAQ 使用追蹤與統計
  - 總點擊數、語系分佈、趨勢圖表
  - Top 20 熱門 FAQ 排行（顯示中文標題）
  - 本地點擊記錄（最近 100 筆，含中文標題）
- ✅ **雪場瀏覽分析** - 用戶雪場互動追蹤
  - 地區點擊分佈（新潟、長野、北海道等）
  - 互動類型分析（展開詳情、官網連結、反饋提交）
  - Top 10 熱門雪場排行
  - 每日瀏覽趨勢圖表
- ✅ **反饋分析系統** - 全面的用戶滿意度分析
  - 反饋統計（總數、有幫助率、類型分佈）
  - 負面反饋原因分析（百分比圖表）
  - 滿意度最高內容 Top 10（FAQ 顯示中文標題）
  - 滿意度最低內容 Bottom 10（<50% 滿意度警示）
  - 最近 50 筆反饋記錄（FAQ 顯示中文標題）

**改進**:
- ✅ **多層降級策略** - API 失敗時自動降級處理
  - Level 1: 聚合統計 API（/feedback-stats, /feedback-reasons 等）
  - Level 2: 從詳細記錄 API 計算統計（/feedback-recent, limit 1000）
  - Level 3: 從 localStorage 計算統計
  - Level 4: 顯示友好空狀態
- ✅ **FAQ 知識庫快取** - 自動載入並快取 FAQ 中文標題
  - 使用 `/api/v1/faq/all` 端點載入全部 FAQ
  - 記憶體快取避免重複 API 請求
  - 所有分析頁面統一顯示中文標題而非 ID
- ✅ **欄位類型處理** - 相容多種資料格式
  - `helpful` 欄位支援數值 (0/1) 和布林值 (true/false)
  - 日期欄位統一使用 ISO 8601 格式
  - 語系代碼標準化（zh/en/th）
- ✅ **localStorage 資料保護** - 資料損壞自動處理
  - JSON 解析錯誤自動重置
  - 無效資料自動過濾
  - 資料降級保證系統穩定

**修復**:
- ✅ 修復反饋統計顯示 0 的問題（API 失敗時改用計算）
- ✅ 修復 FAQ 知識庫載入 400 錯誤（改用 /faq/all 端點）
- ✅ 修復負面反饋原因分析空白問題
- ✅ 修復滿意度 Top/Bottom 內容空白問題
- ✅ 修復本地 FAQ 點擊記錄只顯示 ID 的問題
- ✅ 修復反饋項目未顯示中文標題的問題

**技術債務清理**:
- 🔧 統一 XSS 防護（所有使用者輸入使用 escapeHtml）
- 🔧 標準化錯誤處理（一致的空狀態提示訊息）
- 🔧 改進日誌輸出（明確的功能標籤和錯誤追蹤）
